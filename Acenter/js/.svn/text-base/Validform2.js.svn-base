...
(function($){
	...
	$.fn.Validform.sn={
		...
		regcheck:function(type,gets){
			...
			switch(type){
				case "*": /* 确认密码 */
					return true;
				case "*6-20": /* 输入密码6-20位之间*/
					reg= /[^\s]{6,20}/;
					return reg.test(gets);
				case "*2-15": /* 输入密码6-20位之间*/
					reg= /[^\s]{2,15}/;
					return reg.test(gets);
				case "*2-50": /* 输入密码6-20位之间*/
					reg= /[^\s]{2,50}/;
					return reg.test(gets);
				case "n": /* 数字输入*/
					return !isNaN(gets);
				...
				case "i": /* 身份证 */
					...
					return reg.test(gets);
				case "mz": /* 英文数字中文   验证2-20个 */
                                	reg=/^[\u4e00-\u9fa5_a-zA-Z0-9]{2,20}$/
				//	reg = /^[\u4e00-\u9fa5]{2,10}$/
					return reg.test(gets);	
				case "mv": /* 英文数字中文   验证2-15个 */
                    reg=/^[\u4e00-\u9fa5_a-zA-Z0-9]{2,15}$/
					return reg.test(gets);	
				case "mc": /* 英文数字中文   验证2-50个 */
                    reg=/^[\u4e00-\u9fa5_a-zA-Z0-9]{2,50}$/
					return reg.test(gets);	
	...
	//产生随机数
			...
			if(obj.is("[datatype='random']")){  //判断select表单元素;
				if(obj.val()!=rresult){
				  errorobj=obj;
				  this.showmsg(errormsg,tiptype,{obj:obj},show);
				  return false;
