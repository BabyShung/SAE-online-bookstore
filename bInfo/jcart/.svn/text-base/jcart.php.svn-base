...
class Jcart {
	...
	function __construct() {	//构造函数
		// 获取$config array
		include_once('../bInfo/jcart/config-loader.php');//显示的格式、文字
		$this->config = $config;
	}
	/**
	* Get cart contents
	...
	*/
	public function get_contents() {
		...
		foreach($this->items as $tmpItem) {
			...
			$item['img']      = $this->imgs[$tmpItem];//img
	...
	* @return mixed
	*/
	private function add_item($id, $name, $price, $qty = 1, $url,$img)
	{
		$validPrice = false;
		$validQty = false;
	...
	*/
	public function display_cart() {
		...
		// Add an item
		if ($_POST[$add]) {
			$itemAdded = $this->add_item($id, $name, $price, $qty, $url,$img);
		...
		if ($isCheckout === true) {
			...
			$checkout ='../bInfo/'.$path . '/gateway.php';
		}
		...
		echo tab(1) . "<form method='post' action='$checkout' class='topaypal' name='myform'>\n";
		echo tab(2) . "<fieldset>\n";
		...
		if($this->itemCount > 0) {
			...
			// Display line items        函数：display-cart 1!!!!!!!!!
			foreach($this->get_contents() as $item)	{
				echo tab(5) . "<tr>\n";
				echo tab(6) . "<td class='jcart-item-qty'>\n";
				echo tab(7) . "<input name='jcartItemId[]' type='hidden' value='{$item['id']}' />\n";
				echo tab(7) . "<input id='jcartItemQty-{$item['id']}' name='jcartItemQty[]' size='1' type='text' value='{$item['qty']}' />\n";
				echo tab(6) . "</td>\n";
				echo tab(6) . "<td class='jcart-item-img'>\n";
				echo tab(6) . "<a href='{$item['url']}'><img src='{$item['img']}'  width='35px' height='40px' alt='加载中' class='shadow'/></a>\n";
				echo tab(6) . "<input name='jcartItemImg[]' type='hidden' value='{$item['img']}' />\n";
				echo tab(6) . "</td>\n";
				...
				echo tab(6) . "</td>\n";
				echo "<td class='jcart-item-dlt'>\n";
				echo "<a class='jcart-remove' href='?jcartRemove={$item['id']}' >{$config['text']['removeLink']}</a>\n";
				echo "</td>\n";
				echo tab(5) . "</tr>\n";
			}
		}
	...
	public function display_cart2() {
		...
		if($this->itemCount > 0) {
			...
			// Display line items     display-cart-2 !!!
			foreach($this->get_contents() as $item)	{
				echo tab(5) . "<tr>\n";
				echo tab(6) . "<td class='jcart-item-qty'>\n";
				echo tab(7) . "<input name='jcartItemId[]' type='hidden' value='{$item['id']}' />\n";
